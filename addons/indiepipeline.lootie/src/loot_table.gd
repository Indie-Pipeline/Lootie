		...
		match loot_table_data.probability_type:
			LootTableData.ProbabilityMode.Weight:
				...
				if loot_table_data.fixed_items_per_loot > 0:
					while items_rolled.size() < loot_table_data.fixed_items_per_loot and not mirrored_items.is_empty():
						items_rolled.append_array(roll_items_by_weight(mirrored_items))
...
func roll_items_by_weight(selected_items:  Array[LootItem] = mirrored_items) -> Array[LootItem]:
	...
	return items_rolled if loot_table_data.choose_all_possible_candidates_each_roll_weight else PluginUtilities.pick_random_values(items_rolled, loot_table_data.number_of_items_that_can_be_selected_per_roll_weight)
	
	
func roll_items_by_tier(selected_items:  Array[LootItem] = mirrored_items, selected_min_roll_tier: float = loot_table_data.min_roll_tier, selected_max_roll_tier: float = loot_table_data.max_roll_tier) -> Array[LootItem]:
	var items_rolled: Array[LootItem] = []
	var item_rarity_roll = randf_range(
		selected_min_roll_tier, 
		clampf(selected_max_roll_tier, 0, loot_table_data.max_current_rarity_roll()) if loot_table_data.limit_max_roll_tier_to_maximum_from_loot_table_data.available_items else selected_max_roll_tier
		)

	var current_roll_items = loot_table_data.items_with_rarity_available(selected_items).filter(
		func(item: LootItem):
	...
	return items_rolled if loot_table_data.choose_all_possible_candidates_each_roll_tier else PluginUtilities.pick_random_values(items_rolled, loot_table_data.number_of_items_that_can_be_selected_per_roll_tier)
